 ...
 * 06/3/18 - Fixed bug where restricted files were not being ignored
 *
 */
(function ($) {
    var feature = {};
        ...
        function createCutomInputFile(obj, group, s, uploadLabel) {
            ...
            fileInput.change(function () {
                ...
                if (this.files) //support reading files
                {
                    ...
                    {
                        if (isFileTypeAllowed(obj, s, this.files[i].name) && (s.maxFileSize == -1 || s.maxFileSize == 0 || this.files[i].size <= s.maxFileSize)) {
                          fileArray.push(this.files[i].name);
                        } else {
                          jQuery('#' + fileUploadId).remove();
                        }
                    }
