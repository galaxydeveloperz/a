...
$CPAGE = mswSQL_table('pages', 'id', (int) $_GET['pg'], 'AND `enPage` = \'yes\'', '*');
...
// Load template..
if ($CPAGE->tmp && file_exists('content/' . MS_TEMPLATE_SET . '/custom-templates/' . $CPAGE->tmp)) {
  $tpl->display('content/' . MS_TEMPLATE_SET . '/custom-templates/' . $CPAGE->tmp);
} else {
