...
?>
  <div class="container margin-top-container min-height-container push" id="mscontainer">
    ...
    <div class="row ticketbuttonarea">
      ...
      // Users in dispute..
      include(BASE_PATH . 'control/classes/class.tickets.php');
      $MST            = new tickets();
      $usersInDispute = $MST->disputeUserNames($SUPTICK, $SUPTICK->name);
      $dlinks         = array();
      foreach ($usersInDispute AS $uiDSi => $uiDS) {
        $dlinks[] = array(
          'link' => (in_array('accounts',$userAccess) ||  USER_ADMINISTRATOR == 'yes' ? '?p=accounts&amp;edit=' . ($uiDSi > 0 ? $uiDSi : $SUPTICK->visitorID) : '#'),
          'icon' => '<i class="fa fa-' . ($uiDSi > 0 ? 'check' : 'user') . ' fa-fw highlight_icon"></i> ',
          'name' => $uiDS
        );
        if ($uiDSi == 0) {
          $dlinks[] = array(
      ...
      $links = array(
        array('link' => 'sep'),
        array('link' => '?p=view-dispute&amp;disputeUsers=' . $tickID, 'name' => $msg_disputes8, 'icon' => '<i class="fa fa-edit fa-fw highlight_icon"></i> ')
      );
      echo $MSBOOTSTRAP->button(array(
        'text' => $msadminlang3_1adminviewticket[8],
        'links' => array_merge($dlinks,$links),
        'orientation' => ' dropdown-menu-right',
        'centered' => 'yes',
        'area' => 'admin',
        'icon' => 'user'
      ...
      if (in_array($SUPTICK->ticketStatus, array('close','closed'))) {
        $links = array(
          array(
            'link' => '?p=view-dispute&amp;id=' . $tickID . '&amp;act=reopen',
            'name' => $msadminlang3_1adminviewticket[6],
        ...
        if ($SUPTICK->spamFlag == 'no') {
          if (USER_CLOSE_PRIV == 'yes' || USER_LOCK_PRIV == 'lock') {
            ...
            if ($howManyCustomStats > 3) {
              $links[] = array(
                ...
                'extra' => 'onclick="iBox.showURL(\'?p=view-dispute&amp;id=' . $tickID. ' &amp;showStatuses=yes\',\'\',{width:' . IBOX_STATUSES_WIDTH . ',height:' . IBOX_STATUSES_HEIGHT . '});return false"'
              );
            }
          }
        } else {
          ...
          if (USER_DEL_PRIV == 'yes') {
            ...
            $links[] = array(
              'link' => '?p=view-dispute&amp;id=' . $tickID . '&amp;act=spam-del',
        ...
        if ($SUPTICK->spamFlag == 'no') {
          if ($SETTINGS->disputes == 'yes') {
            ...
            $links[] = array(
              ...
              'name' => $msg_disputes4,
              'icon' => '<i class="fa fa-support fa-fw highlight_icon"></i> ',
              'extra' => 'onclick="mswLinkOp(this.href);return false;"'
            );
          }
        }
