<?php if (!defined('PARENT') || !isset($C_STAT->id)) { exit; }
...
?>
  <div class="container margin-top-container min-height-container push" id="mscontainer">
    ...
    <ol class="breadcrumb">
      ...
      if (isset($_GET['keys'])) {
      ?>
      <li><a href="?t_status=<?php echo $C_STAT->id; ?>"><?php echo mswSH($C_STAT->name); ?></a></li>
      <li class="active"><?php echo $mspubliclang3_7[7]; ?> (<?php echo mswNFM($sqlQryRows); ?>)</li>
      <?php
      } else {
      ?>
      <li class="active"><a href="?t_status=<?php echo $C_STAT->id; ?>"><?php echo mswSH($C_STAT->name); ?></a> (<?php echo mswNFM($sqlQryRows); ?>)</li>
      <?php
      }
      ?>
    </ol>
