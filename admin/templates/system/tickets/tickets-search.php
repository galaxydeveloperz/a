...
if (isset($_GET['keys'])) {
  // Filters..
  if ($_GET['keys']) {
    $_GET['keys']  = preg_replace('/[^0-9a-zA-Z\-\s]/', '', (string) $_GET['keys']);
    ...
    $filters[0] = (ctype_digit($_GET['keys']) ? "`" . DB_PREFIX . "tickets`.`id` = '" . (int) $_GET['keys'] . "'" : "LOWER(`" . DB_PREFIX . "tickets`.`tickno`) LIKE '%" . $_GET['keys'] . "%' OR LOWER(`" . DB_PREFIX . "portal`.`name`) LIKE '%" . $_GET['keys'] . "%' OR LOWER(`" . DB_PREFIX . "tickets`.`subject`) LIKE '%" . $_GET['keys'] . "%' OR LOWER(`" . DB_PREFIX . "tickets`.`ticketNotes`) LIKE '%" . $_GET['keys'] . "%' OR LOWER(`email`) LIKE '%" . $_GET['keys'] . "%' OR LOWER(`comments`) LIKE '%" . $_GET['keys'] . "%'");
    // Are we also searching responses?
    if (isset($_GET['responses']) && !ctype_digit($_GET['keys'])) {
      $ticketIDs = array();
      ...
      while ($RP = mswSQL_fetchobj($q)) {
	    ...
	    if (!empty($ticketIDs)) {
	      $filters[0] = $filters[0].' OR `' . DB_PREFIX . 'tickets`.`id` IN(' . mswSQL(implode(',', $ticketIDs)) . ')';
	    }
    }
  }
...
?>
  <div class="container margin-top-container min-height-container push" id="mscontainer">
    ...
    <ol class="breadcrumb">
      ...
      ?>
      <li><a href="index.php?p=search"><?php echo $msg_adheader7; ?></a></li>
      <li class="active"><?php echo $msg_search6.' ('.mswNFM($sqlQryRows).')'; ?></li>
      <?php
      } else {
      ?>
      <li class="active"><a href="index.php?p=search"><?php echo $msg_adheader7; ?></a></li>
      <?php
      }
      ?>
    </ol>
    ...
    <div class="row searcharea"<?php echo (isset($_GET['keys']) ? ' style="display:none"' : ''); ?>>
          ...
          <div class="panel-body">
            <div class="tab-content">
              <div class="tab-pane active in" id="one">
                ...
                <div class="form-group">
                  <div class="checkbox">
                    <label><input type="checkbox" name="responses" value="yes"<?php echo (isset($_GET['responses']) ? ' checked="checked"' : (!isset($_GET['responses']) && SEARCH_AUTO_CHECK_RESPONSES == 'yes' ? ' checked="checked"' : '')); ?>> <?php echo $msg_search23; ?></label>
                  </div>
                </div>
      ...
      <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 margin_top_10">
        <div class="panel panel-default">
          <div class="panel-heading text-right topfilterarea">
            ...
            <button class="btn btn-info btn-sm" type="button" onclick="mswSearchReload('search')"><i class="fa fa-search fa-fw"></i></button>
