...
$q  = mswSQL_query("SELECT SQL_CALC_FOUND_ROWS *,
      (SELECT count(*) FROM `" . DB_PREFIX . "tickets`
       WHERE (`" . DB_PREFIX . "tickets`.`priority` = 
...
?>
  <div class="container margin-top-container min-height-container push" id="mscontainer">
    ...
    <ol class="breadcrumb">
      ...
      if (isset($_GET['keys'])) {
      ?>
      <li><a href="?p=levelsman"><?php echo $msg_adheader51; ?></a></li>
      <li class="active"><?php echo $mspubliclang3_7[7]; ?> (<?php echo mswNFM($sqlQryRows); ?>)</li>
      <?php
      } else {
      ?>
      <li class="active"><a href="?p=levelsman"><?php echo $msg_adheader51; ?></a> (<?php echo mswNFM($sqlQryRows); ?>)</li>
      <?php
      }
      ?>
    </ol>
      ...
      <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 margin_top_10">
        <div class="panel panel-default">
          <div class="panel-heading text-right">
            <?php
            if (USER_ADMINISTRATOR == 'yes' || in_array('levels', $userAccess)) {
            ?>
            ...
            <div class="table-responsive">
              ...
              <thead>
               <tr>
                 ...
                 <th><?php echo TABLE_HEAD_DECORATION . $msg_levels18; ?></th>
              ...
              <tbody>
                <?php
                if ($sqlQryRows > 0) {
                while ($LEVELS = mswSQL_fetchobj($q)) {
                ?>
                ...
                ?>
                <td><?php echo mswPRMarker(($LEVELS->marker ? $LEVELS->marker : $LEVELS->id), $LEVELS->colors); ?></td>
                <td>
                ...
                <td><b><?php echo mswSH($LEVELS->name); ?></b>
                <span class="tdCellInfo">
                 <?php echo $msg_levels15; ?>: <span class="highlight"><?php echo mswYN($LEVELS->display, $msg_script4, $msg_script5); ?></span>
                </span>
                </td>
                <td><a href="?p=search&amp;keys=&amp;priority=<?php echo ($LEVELS->marker ? $LEVELS->marker : $LEVELS->id); ?>" title="<?php echo mswNFM($LEVELS->tickCount); ?>"><?php echo mswNFM($LEVELS->tickCount); ?></a></td>
                <td class="text-right">
                  <a href="?p=levels&amp;edit=<?php echo $LEVELS->id; ?>" title="<?php echo mswSH($msg_script9); ?>"><i class="fa fa-pencil fa-fw"></i></a>
                </td>
                </tr>
                <?php
                }
          ...
          <?php
	        if (USER_DEL_PRIV == 'yes') {
	        ?>
          <button onclick="mswButtonOp('levdel');return false;" class="btn btn-danger button_margin_right20" disabled="disabled" type="button" id="delButton"><i class="fa fa-trash fa-fw" title="<?php echo mswSH($msg_levels9); ?>"></i> <span class="hidden-xs hidden-sm"><?php echo $msg_levels9; ?></span> <span id="mswCVal">(0)</span></button>
	        <?php
	        }
	        ?>
	        <button class="btn btn-primary" type="button" onclick="mswProcess('levseq')"><i class="fa fa-sort-numeric-asc fa-fw" title="<?php echo mswSH($msg_levels8); ?>"></i> <span class="hidden-xs hidden-sm"><?php echo $msg_levels8; ?></span></button>
          </div>
	        <?php
	        }
          ?>
